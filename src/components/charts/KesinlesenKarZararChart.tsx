...
interface KesinlesenKarZararChartProps {
  gelirler: Gelir[];
  giderler: Gider[];
}

export default function KesinlesenKarZararChart({ gelirler, giderler }: KesinlesenKarZararChartProps) {
  // Grafik verisi
  const chartData = useMemo(() => {
    ...
    const karZararData = labels.map((_, i) => aylikKesinlesenGelir[i] - aylikKesinlesenGider[i]);

    return {
      labels,
      datasets: [
        {
          label: 'Kesinleşen Kar/Zarar',
          data: karZararData,
          backgroundColor: karZararData.map(value => 
            value >= 0 ? 'rgba(16, 185, 129, 0.9)' : 'rgba(220, 38, 127, 0.9)'
          ),
          borderColor: karZararData.map(value => 
            value >= 0 ? 'rgba(16, 185, 129, 1)' : 'rgba(220, 38, 127, 1)'
          ),
          borderWidth: 1,
          borderRadius: 4,
        },
  ...
  const chartOptions = useMemo(() => ({
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Aylık Kesinleşen Kar/Zarar Durumu',
        font: {
          size: 17,
