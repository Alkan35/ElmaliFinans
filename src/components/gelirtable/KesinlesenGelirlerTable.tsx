...
interface KesinlesenGelirlerTableProps {
  gelirler: Gelir[];
  currentPage: number;
  itemsPerPage: number;
...
export default function KesinlesenGelirlerTable({ 
  gelirler, 
  currentPage, 
  itemsPerPage, 
  onPageChange 
}: KesinlesenGelirlerTableProps) {
  const [isModalOpen, setIsModalOpen] = React.useState(false);
  const [selectedGelir, setSelectedGelir] = React.useState<Gelir | null>(null);
  ...
  const handleOdemeClick = (gelir: Gelir) => {
    setSelectedGelir(gelir);
    setIsModalOpen(true);
  };

  const getRemainingInstallments = (gelir: Gelir) => {
    if (gelir.tur === 'taksitli' && gelir.toplamTaksitSayisi && gelir.taksitSirasi) {
      return gelir.toplamTaksitSayisi - gelir.taksitSirasi;
    }
    return gelir.kalanAy || '-';
  };
              ...
              <thead>
                <tr className="bg-gradient-to-r from-gray-50 to-gray-100/50">
                  ...
                  <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 tracking-wider">
                    <div className="flex items-center space-x-2">
                      <svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2M7 4h10M7 4v16a2 2 0 002 2h6a2 2 0 002-2V4M7 8h10M7 12h4" />
                      </svg>
                  ...
                  <td colSpan={7} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center justify-center space-y-4">
                      ...
                      <div className="text-center">
                        <h3 className="text-lg font-medium text-gray-900 mb-1">Henüz kesinleşen gelir yok</h3>
                ...
                {paginatedGelirler.map((gelir) => (
                  <tr key={gelir.id} className="group hover:bg-blue-50/30 transition-all duration-200 border-b border-gray-100/50">
                      ...
                      <div className="flex items-center">
                        {gelir.tur === 'taksitli' ? (
                          <span className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-blue-100 text-blue-800">
                            ...
                            {getRemainingInstallments(gelir)}
                          </span>
                        ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <button
                        onClick={() => handleOdemeClick(gelir)}
                        className="group/btn inline-flex items-center px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white text-sm font-semibold rounded-xl hover:from-green-600 hover:to-emerald-700 transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl"
                      >
              ...
              <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <p className="text-sm text-gray-700 font-medium">
                    <span className="font-semibold text-blue-600">{startIndex + 1}</span> - <span className="font-semibold text-blue-600">{Math.min(endIndex, totalItems)}</span> arası, 
                    toplam <span className="font-semibold text-blue-600">{totalItems}</span> sonuç
                  </p>
                </div>
